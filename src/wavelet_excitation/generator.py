...
class WaveletSignalGenerator:
    ...
    def __init__(
        ...
        if cutoff_freq is not None and cutoff_freq >= self.nyquist:
            raise ValueError(
                f"cutoff_freq ({cutoff_freq} Hz) must be < Nyquist ({self.nyquist} Hz)."
            )
    ...
    def _generate_target_statistics(self) -> None:
        ...
        if len(phi) > self.num_points:
            warnings.warn(
                ...
                "Target spectrum will be truncated.",
                UserWarning,
            )
    ...
    def _rescale_wavelet_to_bandwidth(self, phi_base: np.ndarray, target_f3db: float) -> np.ndarray:
        ...
        interpolator = interp1d(
            old_indices, phi_base, kind="cubic", fill_value=0.0, bounds_error=False
        )
    ...
    def _generate_single_core(self, max_iter: int, tol: float) -> np.ndarray:
        ...
        if seeds is not None:
            if isinstance(seeds, int):
                ...
                if len(seeds) != num_channels:
                    raise ValueError(
                        f"Length of seed list ({len(seeds)}) must match num_channels ({num_channels})."
                    )
                seed_list = seeds
            else:
    ...
    def _apply_post_filter_to_signal(
        self, signal: np.ndarray, user_cutoff: Optional[float]
    ) -> np.ndarray:
        ...
        if user_cutoff is not None:
            ...
            fc = min(
                self.DEFAULT_PREFILTER_FACTOR * self.cutoff_freq,
    ...
    def plot_diagnostics(self) -> None:
        ...
        if t_norm.max() > t_norm.min():
            t_scaled = (t_norm - t_norm.min()) / (t_norm.max() - t_norm.min()) * (
                s_max - s_min
            ) + s_min
        else:
            t_scaled = t_norm
        ...
        axs[1].hist(
            ...
            histtype="step",
            lw=2,
            label=f"Target ({self.target_pdf_type})",
        )
        ...
        axs[2].loglog(
            f_pos, target_fft_norm, label="Target Envelope", color="red", ls="--", lw=1.5, alpha=0.9
        )

        if self.cutoff_freq:
            axs[2].axvline(
                self.cutoff_freq, color="green", ls=":", label=f"Cutoff {self.cutoff_freq} Hz"
            )
