...
def test_reproducibility(default_gen):
    """Ensures that the same seed produces the exact same signal."""
    # Run 1
    sig1 = default_gen.generate(num_channels=1, seeds=123)
    
    # Run 2 (New instance to be sure)
...
def test_pre_filter_warning(default_gen):
    """Check if warning is issued when filter cutoff is too high."""
    with pytest.warns(UserWarning, match="Filtering skipped"):
        # Create a generator where default filter heuristic would exceed Nyquist
        # (Heuristic is often 1.2 * cutoff)
        gen = WaveletSignalGenerator(dt=1.0, t_sim=10, cutoff_freq=0.45) # Nyquist=0.5
        gen.generate(pre_filter=True)